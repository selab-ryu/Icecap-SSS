...
(function (factory) {
	...
	var // currently active contextMenu trigger
		...
		defaults = {
			...
			// child nodes
			reposition: true,
			// Flag denoting if a second trigger should close the menu, as long
			// as
		...
		handle = {
			...
			layerClick: function (e) {
				...
				setTimeout(function () {
					// If the click is not real, things break:
					// https://github.com/swisnl/jQuery-contextMenu/issues/132
					if(fakeClick){
					...
					if (document.elementFromPoint && root.$layer) {
						...
						// also need to try and focus this element if we're in a
						// contenteditable area,
						// as the layer will prevent the browser mouse action we
						// want
			...
			key: function (e) {
				...
				// If targetZIndex is heigher then opt.zIndex dont progress any
				// futher.
				// This is used to make sure that if you are using a dialog with
				// a input / textarea / contenteditable div
			...
			blurItem: function (e) {
				...
				if (opt.autoHide) { // for tablets and touch screens this needs
									// to remain
		...
		op = {
			...
			processPromises: function (opt, root, promise) {
				...
				function errorPromise(opt, root, errorItem) {
					// User called promise.reject() with an error item, if not,
					// provide own error item.
				...
				function finishPromiseProcess(opt, root, items) {
					...
					root.positionSubmenu.call(opt.$node, opt.$menu); // positionSubmenu,
																		// will
																		// only
																		// do
																		// anything
