...
OSP.Term = function() {
	var Term = this;
	OSP._MapObject.apply(Term);
	
	// term start
	var _Term = function( jsonObject ){
		var _Term = this;
		OSP._MapObject.apply(_Term);	
		
		_Term.name_ = fucntion(name) {
			return _Term.property.apply( _Term, OSP.Util.addFirstArgument(OSP.Constants.Name, arguments));
		};
		
		_Term.type_ = fucntion(type) {
			return _Term.property.apply( _Term, OSP.Util.addFirstArgument(OSP.Constants.Type, arguments));
		};
		
		_Term.nameText_ = fucntion(nameText) {
			return _Term.property.apply( _Term, OSP.Util.addFirstArgument(OSP.Constants.NAME_TEXT, arguments) );
		};
		
		_Term.localizedNameText_ = function( languageId, text ) {
			var nameText = _Term.nameText_();
		...
		_Term.removeNameText_ = function( languageId ) {
			return _Term.removeProperty( OSP.Constants.NAME_TEXT, languageId );
		};

		_Term.localizedNameTextXML_ = function(availableLanguageIds, defaultLanguageId) {
			var nameText = _Term.nameText_();
			if( !nameText ){
				nameText = {};
			}
		...
		_Term.description_ = function( description ) {
			return _Term.property.apply(_Term, OSP.Util.addFirstArgument(OSP.Constants.DESCRIPTION, arguments));
		};
		
		_Term.localizedDescription_ = function( languageId, text ) {
			var description = _Term.description_();
			...
			case 2:
				if( !description ){
					description = {};
					_Term.description_( description );
				}
		...
		_Term.removeLocalizedDescription_ = function( languageId ) {
			return _Term.removeProperty(OSP.Constants.NAME_TEXT, languageId);
		};

		_Term.localizedDescriptionXML_ = function(availableLanguageIds, defaultLanguageId) {
			var description = _Term.description_();
